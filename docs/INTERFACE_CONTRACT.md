...
- A separate `aletheia ui` FastAPI app serves packaged, dependency-free browser
...
- Local hosts work without a token. Real non-loopback serving requires
  `ALETHEIA_UI_TOKEN`. API calls use a Bearer header; the browser keeps the token
...
- Checkpoint inspection resolves only canonical retained manifests/blobs under
  `.aletheia/checkpoints`, rejects symlinks/invalid identifiers, and never restores.
