...
function render() {
  ...
  document.title = `${headings[route][0]} · Babysitter`;
}
function stats() {
  const s = data.stats;
...
function setup(id = setupAgent) {
  ...
  showModal(
    `${modalHeader("A little setup. A lot of oversight.", "Run these commands in your dedicated project worktree.")}<div class="modal-body"><div class="tabs">${data.agents.map((x) => `<button data-agent="${x.id}" class="${x.id === id ? "selected" : ""}">${x.name}</button>`).join("")}</div><div class="numbered"><span>1</span>Configure meaningful verification</div>${commandBlock("babysitter init --test 'python -m pytest -q' \\\n  --typecheck 'python -m mypy src'")}<p>Already initialized? Edit <code>babysitter.json</code> explicitly. Use the test and typecheck commands that actually cover your project.</p><div class="numbered"><span>2</span>Connect ${a.name}</div>${commandBlock(a.command)}<div class="notice">${icon("shield")}<span>${esc(a.note)}</span></div><div class="numbered"><span>3</span>See the evidence</div>${commandBlock("babysitter ui")}<p>Open the printed local URL. This console reads evidence; it does not execute these commands.</p></div>`,
    "setup",
  );
}
function workspaceInfo() {
  showModal(
    `${modalHeader("Your workspace. Your machine.")}<div class="modal-body"><p>${data.mode === "demo" ? "This is an interactive demo with illustrative tasks and checkpoints. It does not read or modify any repository." : "This console is connected to one project and never takes runtime ownership."}</p><h3>Open your own repository</h3><p>Run this in a terminal on the machine with Babysitter installed:</p>${commandBlock("babysitter --root /path/to/your/repo ui")}<p>The default address is <code>http://127.0.0.1:8040</code>. Native agents and the console can run together; only the console is read-only.</p><div class="notice">${icon("lock")}<span>Real non-loopback access requires <code>BABYSITTER_UI_TOKEN</code>. Never expose project evidence publicly. Token entry is held in browser memory only.</span></div></div>`,
    "workspace-info",
  );
}
const guides = [
...
function exportTrace() {
  ...
  a.href = url;
  a.download = `babysitter-${currentTask.task.id}-trace.json`;
  a.click();
